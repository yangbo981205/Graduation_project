...
public class FragmentSmog extends Fragment {
    ...
    private LineChart smogLineChart=null;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        smogLineChart=(LineChart)view.findViewById(R.id.smogLineChart);
                ...
                .execute(new StringCallback() {
                    ...
                    public void onResponse(Call call, String s) {
                        ...
                        for(int i=0;i<map.size();i++){
                            DataSet dataSet=new DataSet(String.valueOf(i+1),map.get(i).get("time"),map.get(i).get("smokescope"));
    ...
    private void initLineChart(){
        ...
        smogLineChart.setDescription(description);
        Legend legend = smogLineChart.getLegend();
    ...
    private void setYAxis(){
        YAxis yAxisLeft = smogLineChart.getAxisLeft();// 左边Y轴
        ...
        // 右侧Y轴
        smogLineChart.getAxisRight().setEnabled(false); // 不启用
    }
    ...
    private void setXAxis(){
        // X轴
        XAxis xAxis = smogLineChart.getXAxis();
    ...
    private void setData(){
        // 数据
        final List<Entry> smog = new ArrayList<>();
        final List<Float> smogData = new ArrayList<>();
        OkHttpUtils.post()
                .url("http://"+SERVERIP+":8080/graduationproject/android/environment")
                .build()
                .execute(new StringCallback() {
                    ...
                    public void onResponse(Call call, String s) {
                        ...
                        List<HashMap<String,String>> map = JSONTOOL.analyze_some_json(getMap.get(0).get("result"));
                        for(int i=0;i<6;i++){
                            smogData.add(Float.valueOf(map.get(i).get("smokescope")));
                        }
                        for (int i=0;i<smogData.size(); i++) {
                            smog.add(new Entry(i, smogData.get(i)));
                        }
                        // 2. 分别通过每一组Entry对象集合的数据创建折线数据集
                        LineDataSet lineDataSet1 = new LineDataSet(smog, "实时可燃物浓度数据");
                        ...
                        //将折线数据设置给图表
                        smogLineChart.setData(lineData);
